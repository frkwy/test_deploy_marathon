...
def deploy(image_name,
           id=None,
           label_dict={'environment': 'staging',
                       'name': 'test1',
                       'deploy_by': 'jenkins'},
           env_dict={},
           args=[],
           cpus=0.1):
    if not id:
        re_id = re.search('(?<=[\w|\.|/])[\d]+$', find_latest().id).group()
        id = str(int(re_id) + 1)
    else:
        id = str(id)
    to_marathon = {
    ...
    r = requests.post(MARATHON_URL + 'v2/apps', json=to_marathon, headers=headers)
    r.raise_for_status()


def remove_container(app_id):
    requests.delete(MARATHON_URL + 'v2/apps/' + app_id)


def remove_all():
    for app in c.list_apps():
        remove_container(app.id)
